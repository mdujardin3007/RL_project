...
class CustomLunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        max_fuel: float = 100.0, # NEW FEATURE
        variable_terrain: bool = False, # NEW FEATURE
        actuator_delay: int = 0, # NEW FEATURE
    ):
        EzPickle.__init__(
            self,
            render_mode,
        ...
        self.actuator_delay = actuator_delay  # Delay Frames
        self.action_buffer = []
        if self.actuator_delay > 0:
            self.action_buffer = [0 if not self.continuous else np.array([0.0, 0.0])] * self.actuator_delay

        self.smooth_landing_bonus_enabled = True
        self.has_landed = False
        self.landing_velocities = None
        ################################
    ...
    def reset(
        ...
        *,
        ...
        ############ NEW FEATURE ############
        if self.variable_terrain:
            offset = self.np_random.uniform(-W * 0.2, W * 0.2)
        else:
            offset = 0
        
        self.helipad_x1 = W / 2 - 0.1 * W + offset
        self.helipad_x2 = W / 2 + 0.1 * W + offset
        self.helipad_y = H / 4
    ...
    def step(self, action):
        ...
        ############ NEW FEATURE ############
        if self.has_landed and not self.lander.awake:
            action = 0 if not self.continuous else np.array([0.0, 0.0])
        ...
        if self.fuel <= 0.0:
            # No more fuel, disable any engine action
            action = 0 if not self.continuous else np.array([0.0, 0.0])
        self._update_gravity_by_mass()
        #####################################
        ...
        self.fuel -= s_power * 0.3
        self.fuel = max(self.fuel, 0.0)
        #####################################

        state = [
            #(pos.x - VIEWPORT_W / SCALE / 2) / (VIEWPORT_W / SCALE / 2), OLD CALCULATION
            ...
            self.fuel / self.max_fuel #################### NEW FEATURE ####################
        ]
        assert len(state) == 9  #################### NEW FEATURE ####################
        ...
        ############ NEW FEATURES ############
        if (not self.has_landed) and self.legs[0].ground_contact and self.legs[1].ground_contact:
            self.has_landed = True
            self.landing_velocities = (abs(vel.y), abs(self.lander.angularVelocity))

        lander_x = self.lander.position.x
        if not self.lander.awake:
            terminated = True
            if self.helipad_x1 <= lander_x <= self.helipad_x2:
                reward += 100 + (self.fuel / self.max_fuel) * 50
                if self.has_landed and self.smooth_landing_bonus_enabled:
                    vertical_v, angular_v = self.landing_velocities
                    smoothness = max(0.0, 1.0 - (vertical_v / 2.0 + abs(angular_v) / 5.0))
                    reward += 5.0 * smoothness
            else:
                reward += -100

        if not self.fuel_ran_out and self.fuel <= 0.0 and not self.has_landed: #We penalize slightly fuel exhaustion
            reward -= 30  # or -20, or any light penalty
        ...
        info = {
                "fuel_ran_out": self.fuel_ran_out,
                "landing_smoothness": {
                    "vertical_velocity": self.landing_velocities[0],
                    "angular_velocity": self.landing_velocities[1],
                    "landed_on_pad": self.helipad_x1 <= lander_x <= self.helipad_x2
                } if self.has_landed and self.landing_velocities is not None else None
            }
        ###################################
    ...
    def render(self):
        ...
        fuel_text = font.render(f"Fuel: {self.fuel:.1f}", True, (255, 255, 255))
        self.surf.blit(fuel_text, (10, 10))  # You can change (10, 10) to another position if needed

        mass_text = font.render(f"Mass factor: {(self.fuel / self.max_fuel) * 0.3 + 1.0:.2f}", True, (255, 255, 255))
        self.surf.blit(mass_text, (10, 30))
    ...
    ################ NEW FEATURE ################
    def _update_gravity_by_mass(self):
        """
        Dynamically adjust gravity based on current fuel level.
        The idea is: less fuel = lighter lander = weaker gravitational pull.
        """
        mass_factor = (self.fuel / self.max_fuel) * 0.3 + 1.0  # stays in [1.0, 1.3]
        adjusted_gravity = self.original_gravity * mass_factor
        self.world.gravity = (0, adjusted_gravity)
    #############################################
